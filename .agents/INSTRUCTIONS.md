...
Engine code is **only** `extras/dcmmlib`, a submodule of `https://github.com/DHANUSH-web/dcmmlib.git` tracking **`main`**. Do not tell contributors to clone dcmmlib as a second repo. `make init` and `git clone --recurse-submodules` must be enough.

`make init` does `submodule update --init` then checks out **`origin/main`**. After engine work lands on dcmmlib **`main`**, run `make init` and commit the gitlink if the app should pick it up.

## Product

Free CleanMyMac-style utility: Smart Scan, Deep Scan, Large Files, Duplicates, Uninstaller, Privacy, Space Lens, Maintenance, Settings. Moves **checked** items to Trash after confirmation. No telemetry.
...
| `src/ui/ResultsView.mm` | Smart Scan / Deep Scan / Privacy lists |
...
- **No maximize / full screen.** Disable the zoom button, `windowShouldZoom:` returns NO, `NSWindowCollectionBehaviorFullScreenNone`. Miniaturize and close stay.
...
1. **Opt-in for Deep Scan, Privacy, Large Files, Duplicates, Uninstaller.** Those lists start **unchecked**.
   **Smart Scan is the exception:** `scanSmart()` returns **recommended groups already selected**. The user still confirms before Trash. Do not list per-file rows on Smart Scan — one row per group (User Caches, Logs, Saved Application State). Deep Scan is the item-by-item picker (`scanJunk`); it is **not** safe to Select All — keep the caution on that page.
...
The engine repo **dcmmlib** develops on **`dev`**. After engine changes, point `extras/dcmmlib` at that `dev` commit (not `main`).
...
- Behavior/safety/scan: change **dcmmlib** on **`dev`**, then sync `extras/dcmmlib`, then wire UI on **`beta`**.
